...
export class LoginPage implements OnInit {
  ...
  async submit () {
    ...
    this.auth.signInWithEmailAndPassword (this.form.value.email, this.form.value.password).then (async (response: firebase.auth.UserCredential) => {
      let usuario: any = await this.database.get_usuario_to_promise (response.user.uid);
      ...
      if (usuario.tipo === 1) {
        ...
        let alert = await this.alertController.create({
          header: 'Opppps!',
          message: 'Ingrese con una cuenta de repartidor',
          buttons: ['OK']
        });
